...
class Model(object):
	...
	def __init__(self, configs):
		...
		self.x = [tf.placeholder(tf.float32, [
				...
				(self.configs.input_seq_length + self.configs.output_seq_length) // self.configs.dimension_3D,
				self.configs.img_width,
				self.configs.img_width,
				self.configs.dimension_3D]) for i in range(self.configs.n_gpu)
				]

		self.real_input_flag = tf.placeholder(tf.float32, [
			self.configs.batch_size,
			self.configs.output_seq_length / self.configs.dimension_3D - 1,
			self.configs.img_width,
			self.configs.img_width,
			self.configs.dimension_3D
			])
		...
		for i in range(self.configs.n_gpu):
			with tf.device('/gpu:%d' % i): #TODO: Check whether this way of specifying gpu is correct.
	...
	def construct_model(self, images, real_input_flag, num_layers, num_hidden):
		"""Contructs a model."""
		networks_map = {'e3d_lstm': eidetic_3d_lstm_net.rnn, }
