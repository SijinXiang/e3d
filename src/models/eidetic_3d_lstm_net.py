...
def rnn(images, real_input_flag, num_layers, num_hidden, configs):
	...
	input_length = configs.input_seq_length // configs.dimension_3D
	...
	for i in range(num_layers):
		...
		new_lstm = eidetic_lstm(
			...
			kernel_shape = [1, filter_size, filter_size]) # 5是不是可以用configs的filter size?

		lstm_layer.append(new_lstm)
		zero_state = tf.zeros(
			[batch_size, ims_depth, ims_width, ims_height, num_hidden[i]]) # 为什么第二维是window_length
	...
	with tf.variable_scope('generator'):
		...
		for time_step in range(total_length - 1):
			with tf.variable_scope('e3d-lstm', reuse=reuse):
				...
				# input_frm = tf.transpose(input_frm, [1, 0, 2, 3, 4, 5])
				...
				x_gen = tf.layers.conv3d(hidden[-1], output_channels,
	...
	print('gen_images shape: ', tf.shape())
