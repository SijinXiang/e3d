  ...
  Beyond.", ICLR (2019). https://openreview.net/pdf?id=B1lKS2AqtX
    ...
    Raises:
      ValueError: If any number of dimensions regarding the inputs is not 4 or 5
		...
		shape_channels = 100
		query = tf.reshape(in_query, [batch, -1, shape_channels])
		keys = tf.reshape(in_keys, [batch, -1, shape_channels])
		values = tf.reshape(in_values, [batch, -1, shape_channels])
		attn = tf.matmul(query, keys, False, True) #Transpose keys for matrix multiplication
		'''
		print('numchannel', num_channels)
		print('attn', attn)
		...
		attn = tf.matmul(attn, values, False, False) #No transposition
		if len(q_shape) == 4:
	...
	def __call__(self, inputs, hidden, cell, global_memory, eidetic_cell):
		with tf.variable_scope(self._layer_name):
			...
			i_x, g_x, r_x, o_x, temp_i_x, temp_g_x, temp_f_x = tf.split(
				value=new_inputs, num_or_size_splits=7, axis=-1) #Original code has indentation(suo jin) but probably wrong
			...
			#new_cell = cell + r_t
			new_cell = self._norm(new_cell, "self_attn") + i_t * g_t #the memory state from previous state Ctk

			new_global_memory = self._conv(global_memory, 4 * self._output_channels,
                                     self._kernel_shape) #global memory is the zifzag memory Mt.k-1

			if self._layer_norm:
				new_global_memory = self._norm(new_global_memory, "global_memory")
			i_m, f_m, g_m, m_m = tf.split(
				value=new_global_memory, num_or_size_splits=4, axis=-1) #Original code has indentation(suo jin) but probably wrong
			...
			output = tf.tanh(memory) * tf.sigmoid(output_gate) #output is the desired hidden state expression

		return output, new_cell, global_memory #返回hidden state, memory Ctk, global zifzag memory
