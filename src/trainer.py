...
def test(model, test_input_handle, configs, save_name):
	...
	batch_id = 0
	img_mse, ssim, psnr = [], [], []
	output_length = configs.output_seq_length // configs.dimension_3D
	...
	real_input_flag_zero = np.zeros((configs.batch_size, output_length - 1,
									...
									configs.img_width // configs.patch_size,
									configs.patch_size**2 * configs.dimension_3D))
	...
	while not test_input_handle.no_batch_left():
		...
		'''for i in range((configs.output_seq_length + configs.input_seq_length) / configs.dimension_3D):
                if (i + 1) < 10:
           	 ...
           	 for i in range(output_length):
                if (i + configs.input_seq_length / configs.dimension_3D + 1) < 10:
                    name = 'pd0' + str(i + configs.input_seq_length / configs.dimension_3D + 1) + '.png'
                else:
                    name = 'pd' + str(i + configs.input_seq_length / configs.dimension_3D + 1) + '.png'
                file_name = os.path.join(path, name)
                img_pd = img_gen[0, i]
	...
	avg_mse = avg_mse / configs.output_seq_length
	avg_result[1] = avg_mse
	print('mse per frame: ' + str(avg_mse))
	name = 'avg:seq_frm.txt'
